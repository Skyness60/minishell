...
/*   Updated: 2024/10/25 11:58:21 by sperron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

int	unset_error(char *argv, int status, bool is_child)
{
	if (is_child == true)
	{
		if (status == 0)
...
void	unset_one(t_data *data, char *arg)
{
	...
	while (data->env[i])
	{
		if (!ft_strncmp(data->env[i], arg, length) \
		&& data->env[i][length] == '=')
		{
			pointer = &data->env[i];
			to_free = data->env[i];
...
int	handle_unset(t_data *data, char **args, bool is_child, int fd)
{
	...
	{
		if (args[1][0] == '-')
			out = unset_error(args[1], 1, is_child);
		else if (!ft_isalpha(args[i][0]) && args[i][0] != '_')
			out = unset_error(args[i], 0, is_child);
